...
def clean_main_logs(main_log_directory, prepend="rapid_", log_file_path=""):
    ...
    for main_log_file in main_log_files:
        try:
            # MJS log_datetime = datetime.datetime.strptime(main_log_file[:18],
            # MJS                                          "{0}%y%m%d%H%M%S".format(
            # MJS                                              prepend))
            log_datetime = datetime.datetime.strptime(main_log_file[18:-4], "{0}%y%m%d%H%M%S".format(prepend))
