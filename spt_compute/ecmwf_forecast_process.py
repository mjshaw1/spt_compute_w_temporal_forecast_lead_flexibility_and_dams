...
from .imports.generate_warning_points import generate_ecmwf_warning_points, generate_lsm_warning_points
...
# MJS                                                     'Runoff.' + date_string + '*.netcdf')))
                                                     'Runoff*' + date_string + '*netcdf')))
            print('ECMWF forecast location: ',ecmwf_forecast_location)
            print('ECMWF folders: ',ecmwf_folders)
        # LOAD LOCK INFO FILE
        ...
        if os.path.exists(LOCK_INFO_FILE):
            ...
            if previous_lock_info['running']:
                ...
                for ecmwf_folder in ecmwf_folders:
                    ...
                    print(forecast_date,last_forecast_date)
                ...
                for rapid_input_directory in rapid_input_directories:
                    ...
                    for watershed_job_index, forecast in enumerate(ecmwf_forecasts):
                        ...
                        rapid_watershed_jobs[rapid_input_directory]['jobs_info'].append({'watershed': watershed,
                                                                                         ...
                                                                                         # MJS 'data_manager':data_manager # added this to try to upload forecast in mp
                                                                                         })
                        print(mp_mode)
                        if mp_mode == "htcondor":
                            ...
                            print('Going to process Qout now....')
...
# dam arguments included, MJS 8/23/2020 ........
                            ...
                            ##                                                         watershed_job_index
                            ##                                                         initialization_time_step))
                            print('Aparently processed Qout?')
                        else:
                ...
                for rapid_input_directory, watershed_job_info in rapid_watershed_jobs.items():
                    ...
                    if mp_mode == "htcondor":
                        ...
                        pool_main = mp_Pool()
                        # MJS func = partial(run_ecmwf_rapid_multiprocess_worker, watershed_job_info['jobs_info'])
                        func = partial(run_ecmwf_rapid_multiprocess_worker, watershed_job_info['jobs_info'])
                        # MJS multiprocess_worker_list = pool_main.imap_unordered(func,
                        ...
                        pool_main.close()
                        pool_main.join()
                    print("Got through the forecast it seems")
                    ...
                    if create_warning_points:
                        ...
                        if os.path.exists(era_interim_watershed_directory):
                            ...
                            if era_interim_files:
                                try:
                                    #generate_ecmwf_warning_points(watershed, subbasin, era_interim_watershed_directory, era_interim_files[0],
                                    #                              master_watershed_outflow_directory, tethys_url, tethys_directory,
                                    #                              tethys_username,tethys_keyfilename,forecast_date_timestep,
                                    #                              threshold=warning_flow_threshold)
                                    #generate_lsm_warning_points(forecast_file, era_interim_files[0],
